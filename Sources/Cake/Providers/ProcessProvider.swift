...
actor LiveProcessRegistry {
    ...
    static func scanClaudeProcesses() -> [ClaudeProcessInfo] {
        ...
        for i in 0..<actualCount {
            ...
            // 优先用可执行路径分类。但被 hardened-runtime 签名的 .app 进程（如 Kiro CLI 的
            // kiro-cli-chat）proc_pidpath 会返回 0（无路径读取权限）——此时回退用 proc_name
            // 读 comm 名分类，否则 Kiro 主进程永远枚举不到 → 刘海不显示、tty 跳转失效。
            ...
            if len > 0 {
                ...
                kind = agentKind(forPath: path)
            } else {
                kind = agentKind(forName: procName(of: pid))
            }
    ...
    /// 基于 comm 名（proc_name，p_comm）分类——仅在 proc_pidpath 拿不到路径时的兜底。
    /// 注意 p_comm 截断到 MAXCOMLEN(16) 字节："kiro-cli-chat"(13) 未超、完整可见；
    /// "claude"/"codex" 同理。仍用精确名匹配避免误判（"Claude Helper" 的 comm 不是 "claude"）。
    static func agentKind(forName name: String?) -> AgentKind? {
        ...
        if lower == "claude" { return .claudeCode }
        return nil
    }

    /// 进程 comm 名（p_comm，≤16 字节）。proc_pidpath 失败时用它兜底分类。
    static func procName(of pid: pid_t) -> String? {
        var buf = [CChar](repeating: 0, count: 2048)
        let len = proc_name(pid, &buf, 2048)
