...
struct KiroProvider: AgentProvider {
    ...
    static func parseSession(jsonFile: URL) -> SessionSnapshot? {
                ...
                if let cm = ss["conversation_metadata"] as? [String: Any],
                   let turns = cm["user_turn_metadatas"] as? [[String: Any]] {
                    ...
                    if let last = turns.last {
                        lastTurnEnded = (last["end_reason"] as? String)?.isEmpty == false
                    }
                }
            }
        }
        ...
        if pidAlive {
            // 进程存活时，用"最后一个 turn 是否已结束"区分真在跑 vs 完成。
            // Kiro 是交互式 REPL，任务完成后进程仍活着等下一句——不能据此判 running。
            if lastTurnEnded {
                state = .done                 // 轮次已结束 = 任务完成（与 Claude 完成同图标 ✓）
            } else {
                state = age < 10 ? .running : .done   // 轮次进行中：近期有输出=running
            }
        } else {
    ...
    /// 该 pid 的可执行路径是否是 kiro-cli（避免 PID 重用导致的误判）。
