...
struct OTelLogProvider: AgentProvider {
    ...
    /// bySession 会话条目的存活上限：超过此时长无新事件即清理，防内存无界增长。
    static let sessionTTL: TimeInterval = 3600   // 1 小时
            ...
            let task = Task.detached(priority: .utility) {
                ...
                while !Task.isCancelled {
                    ...
                    for ev in events {
                        ...
                        // OTel 只累加 token，不读 stop_reason → 非权威，state 不应覆盖 JSONL 判定。
                        snap.hasAuthoritativeState = false
                        snap.model = ev.model
                    ...
                    // 老化清理：移除长时间无新事件的会话，避免 bySession 随历史会话无界增长
                    // （进程常驻、跨数百会话时会泄漏）。安全性：OTel 是 per-request 增量累加，
                    // 但聚合层 ingest 以两源 token 较大值为准，即便某会话被清后又来新事件、本地
                    // 从 0 重累加，显示总量仍由聚合层保留的旧大值兜底，不会回退。TTL 取 1 小时，
                    // 远大于活跃会话的事件间隔，不会误清正在跑的会话。
                    if !events.isEmpty {
                        let cutoff = Date().addingTimeInterval(-Self.sessionTTL)
                        bySession = bySession.filter { $0.value.lastActivity >= cutoff }
                    }
    ...
    static func readNewEvents(path: URL, offset: inout UInt64) -> [APIRequestEvent] {
        ...
        // 只处理到【最后一个换行符】为止——尾部可能是 Claude 正在写、尚未写完的半行。
        // offset 只推进到该换行之后，下次从这里续读，避免把半行算进 offset 而永久丢失它。
        let newline = UInt8(ascii: "\n")
        guard let lastNL = data.lastIndex(of: newline) else {
            return []   // 这批数据里还没有完整行，本次不推进 offset，下次重读
        }
